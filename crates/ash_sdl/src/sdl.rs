...
extern "C" {
    ...
    fn SDL_CreateWindow(
        title: *const c_char, x: c_int, y: c_int, w: c_int, h: c_int, flags: u32,
    ) -> *mut c_void;
    ...
    fn SDL_ShowSimpleMessageBox(flags: u32, title: *const c_char, msg: *const c_char, win: *mut c_void) -> c_int;
    fn SDL_Delay(ms: u32);
...
struct SDL_KeyboardEvent { type_: u32, timestamp: u32, window_id: u32, state: u8, repeat: u8, _pad: u16, scancode: i32, sym: i32, _mod: u16 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_MouseMotionEvent { type_: u32, timestamp: u32, window_id: u32, which: u32, state: u32, x: i32, y: i32, xrel: i32, yrel: i32 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_MouseButtonEvent { type_: u32, timestamp: u32, window_id: u32, which: u32, button: u8, state: u8, clicks: u8, _pad: u8, x: i32, y: i32 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_MouseWheelEvent { type_: u32, timestamp: u32, window_id: u32, which: u32, x: i32, y: i32, direction: u32 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_WindowEvent { type_: u32, timestamp: u32, window_id: u32, event: u8, _pad: [u8; 3], data1: i32, data2: i32 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_TextInputEvent { type_: u32, timestamp: u32, window_id: u32, text: [u8; 32] }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_TouchFingerEvent { type_: u32, timestamp: u32, touch_id: i64, finger_id: i64, x: f32, y: f32, dx: f32, dy: f32, pressure: f32, window_id: u32 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_JoyAxisEvent { type_: u32, timestamp: u32, which: i32, axis: u8, _pad: [u8; 3], value: i16 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_JoyBallEvent { type_: u32, timestamp: u32, which: i32, ball: u8, _pad: [u8; 3], xrel: i16, yrel: i16 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_JoyHatEvent { type_: u32, timestamp: u32, which: i32, hat: u8, value: u8 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_JoyButtonEvent { type_: u32, timestamp: u32, which: i32, button: u8, state: u8 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_JoyDeviceEvent { type_: u32, timestamp: u32, which: i32 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_ControllerButtonEvent { type_: u32, timestamp: u32, which: i32, button: u8, state: u8 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_ControllerAxisEvent { type_: u32, timestamp: u32, which: i32, axis: u8, _pad: [u8; 3], value: i16 }
#[repr(C)] #[derive(Copy, Clone)]
struct SDL_DropEvent { type_: u32, timestamp: u32, file: *mut c_char, window_id: u32 }
...
unsafe extern "C" fn sdl_init_once() -> bool {
    ...
    SDL_SetHint(b"SDL_JOYSTICK_ALLOW_BACKGROUND_EVENTS\0".as_ptr() as _, b"1\0".as_ptr() as _);
...
unsafe extern "C" fn sdl_gl_options(major: i32, minor: i32, depth: i32, stencil: i32, flags: i32, samples: i32) {
        ...
        match e.type_ {
            ...
            SDL_TEXTINPUT => {
                ...
                let mask = if t[0] == 0 { 0 }
                    ...
                    else if t[2] == 0 { 0xFFFF }
                    else if t[3] == 0 { 0xFFFFFF }
                    else { 0xFFFFFFFFu32 as i32 };
                code &= mask;
                (*event).key_code = code;
            }
            SDL_FINGERDOWN => {
...
unsafe extern "C" fn sdl_quit() { SDL_Quit(); }
unsafe extern "C" fn sdl_delay(ms: i32) { hl_blocking(true); SDL_Delay(ms as u32); hl_blocking(false); }
unsafe extern "C" fn sdl_detect_win32() -> bool { false }
unsafe extern "C" fn sdl_set_vsync(v: bool) { SDL_GL_SetSwapInterval(if v { 1 } else { 0 }); }
...
unsafe extern "C" fn sdl_get_screen_width_of_window(win: *mut c_void) -> i32 {
    let idx = if !win.is_null() { SDL_GetWindowDisplayIndex(win) } else { 0 };
    let mut mode: SDL_DisplayMode = std::mem::zeroed();
...
unsafe extern "C" fn sdl_set_relative_mouse_mode(enable: bool) -> i32 { SDL_SetRelativeMouseMode(enable as i32) }
unsafe extern "C" fn sdl_get_relative_mouse_mode() -> bool { SDL_GetRelativeMouseMode() != 0 }
unsafe extern "C" fn sdl_warp_mouse_global(x: i32, y: i32) -> i32 { SDL_WarpMouseGlobal(x, y) }
unsafe extern "C" fn sdl_warp_mouse_in_window(win: *mut c_void, x: i32, y: i32) { SDL_WarpMouseInWindow(win, x, y); }
unsafe extern "C" fn sdl_set_window_grab(win: *mut c_void, grabbed: bool) { SDL_SetWindowGrab(win, grabbed as i32); }
unsafe extern "C" fn sdl_get_window_grab(win: *mut c_void) -> bool { SDL_GetWindowGrab(win) != 0 }
unsafe extern "C" fn sdl_get_global_mouse_state(x: *mut i32, y: *mut i32) -> i32 { SDL_GetGlobalMouseState(x, y) as i32 }
unsafe extern "C" fn sdl_detect_keyboard_layout() -> *const u8 { b"qwerty\0".as_ptr() }
unsafe extern "C" fn sdl_show_cursor(show: bool) { SDL_ShowCursor(if show { 1 } else { 0 }); }
unsafe extern "C" fn sdl_is_cursor_visible() -> bool { SDL_ShowCursor(-1) == 1 }

// Window functions
unsafe extern "C" fn sdl_win_create_ex(w: i32, h: i32, x: i32, y: i32, flags: i32) -> *mut c_void {
    eprintln!("[ash_sdl] win_create_ex w={} h={} x={} y={} flags={:#x}", w, h, x, y, flags);
    let mut sdl_flags = SDL_WINDOW_OPENGL | SDL_WINDOW_ALLOW_HIGHDPI;
    if flags & 1 != 0 { sdl_flags |= SDL_WINDOW_RESIZABLE; }
    ...
    let (w, h, x, y) = if w > 10000 && h > 10000 && x > 0 && x < 10000 && y > 0 && y < 10000 {
        eprintln!("[ash_sdl] Detected swapped args: using x={} y={} as w/h", x, y);
        (x, y, w, h)
    } else {
        (w, h, x, y)
    };
    let px = if x == -1 || x > 100000 { SDL_WINDOWPOS_CENTERED } else { x };
    let py = if y == -1 || y > 100000 { SDL_WINDOWPOS_CENTERED } else { y };
...
extern "C" { fn SDL_ShowWindow(window: *mut c_void); fn SDL_PumpEvents(); }
...
unsafe extern "C" fn sdl_win_set_fullscreen(win: *mut c_void, mode: i32) -> bool {
    let flags = match mode { 1 => 0x00000001, 2 => 0x00001001, _ => 0 };
    SDL_SetWindowFullscreen(win, flags) == 0
}
unsafe extern "C" fn sdl_win_set_title(win: *mut c_void, title: *const u8) { SDL_SetWindowTitle(win, title as _); }
unsafe extern "C" fn sdl_win_get_size(win: *mut c_void, w: *mut i32, h: *mut i32) { SDL_GetWindowSize(win, w, h); }
unsafe extern "C" fn sdl_win_set_size(win: *mut c_void, w: i32, h: i32) { SDL_SetWindowSize(win, w, h); }
unsafe extern "C" fn sdl_win_set_position(win: *mut c_void, x: i32, y: i32) { SDL_SetWindowPosition(win, x, y); }
unsafe extern "C" fn sdl_win_get_position(win: *mut c_void, x: *mut i32, y: *mut i32) { SDL_GetWindowPosition(win, x, y); }
unsafe extern "C" fn sdl_win_set_min_size(win: *mut c_void, w: i32, h: i32) { SDL_SetWindowMinimumSize(win, w, h); }
unsafe extern "C" fn sdl_win_set_max_size(win: *mut c_void, w: i32, h: i32) { SDL_SetWindowMaximumSize(win, w, h); }
unsafe extern "C" fn sdl_win_get_min_size(win: *mut c_void, w: *mut i32, h: *mut i32) { SDL_GetWindowMinimumSize(win, w, h); }
unsafe extern "C" fn sdl_win_get_max_size(win: *mut c_void, w: *mut i32, h: *mut i32) { SDL_GetWindowMaximumSize(win, w, h); }
...
unsafe extern "C" fn sdl_win_destroy(win: *mut c_void, ctx: *mut c_void) {
    ...
    if !ctx.is_null() { SDL_GL_DeleteContext(ctx); }
    if !win.is_null() { SDL_DestroyWindow(win); }
}
unsafe extern "C" fn sdl_win_get_id(win: *mut c_void) -> i32 { SDL_GetWindowID(win) as i32 }
unsafe extern "C" fn sdl_win_resize(win: *mut c_void, mode: i32) { /* stub */ }
unsafe extern "C" fn sdl_win_set_display_mode(_w: *mut c_void, _ww: i32, _hh: i32, _r: i32) -> bool { false }
unsafe extern "C" fn sdl_win_display_handle(win: *mut c_void) -> i32 { SDL_GetWindowDisplayIndex(win) }

// Stubs for functions not yet needed
unsafe extern "C" fn sdl_event_poll(_e: *mut c_void) -> i32 { 0 }
unsafe extern "C" fn sdl_set_clipboard_text(_t: *const u8) -> bool { false }
unsafe extern "C" fn sdl_get_clipboard_text() -> *const u8 { b"\0".as_ptr() }
unsafe extern "C" fn sdl_set_drag_and_drop_enabled(_e: bool) {}
unsafe extern "C" fn sdl_get_drag_and_drop_enabled() -> bool { false }
unsafe extern "C" fn sdl_get_displays() -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_get_display_modes(_d: i32) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_get_current_display_mode(_d: i32, _desktop: bool) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_get_devices() -> *mut c_void { ptr::null_mut() }

// Controller/joystick/haptic/cursor stubs
unsafe extern "C" fn sdl_gctrl_count() -> i32 { 0 }
unsafe extern "C" fn sdl_gctrl_open(_i: i32) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_gctrl_close(_c: *mut c_void) {}
unsafe extern "C" fn sdl_gctrl_get_axis(_c: *mut c_void, _a: i32) -> i32 { 0 }
unsafe extern "C" fn sdl_gctrl_get_button(_c: *mut c_void, _b: i32) -> bool { false }
unsafe extern "C" fn sdl_gctrl_get_id(_c: *mut c_void) -> i32 { 0 }
unsafe extern "C" fn sdl_gctrl_get_name(_c: *mut c_void) -> *const u8 { b"\0".as_ptr() }
unsafe extern "C" fn sdl_joy_count() -> i32 { 0 }
unsafe extern "C" fn sdl_joy_open(_i: i32) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_joy_close(_j: *mut c_void) {}
unsafe extern "C" fn sdl_joy_get_axis(_j: *mut c_void, _a: i32) -> i32 { 0 }
unsafe extern "C" fn sdl_joy_get_hat(_j: *mut c_void, _h: i32) -> i32 { 0 }
unsafe extern "C" fn sdl_joy_get_button(_j: *mut c_void, _b: i32) -> bool { false }
unsafe extern "C" fn sdl_joy_get_id(_j: *mut c_void) -> i32 { 0 }
unsafe extern "C" fn sdl_joy_get_name(_j: *mut c_void) -> *const u8 { b"\0".as_ptr() }
unsafe extern "C" fn sdl_haptic_open(_c: *mut c_void) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_haptic_close(_h: *mut c_void) {}
unsafe extern "C" fn sdl_haptic_rumble_init(_h: *mut c_void) -> i32 { -1 }
unsafe extern "C" fn sdl_haptic_rumble_play(_h: *mut c_void, _s: f64, _d: i32) -> i32 { -1 }
unsafe extern "C" fn sdl_cursor_create(_s: *mut c_void, _x: i32, _y: i32) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_cursor_create_system(_id: i32) -> *mut c_void { ptr::null_mut() }
unsafe extern "C" fn sdl_free_cursor(_c: *mut c_void) {}
unsafe extern "C" fn sdl_set_cursor(_c: *mut c_void) {}
unsafe extern "C" fn sdl_surface_from(_b: *const u8, _w: i32, _h: i32, _p: i32, _rm: i32, _gm: i32, _bm: i32, _am: i32, _dep: i32) -> *mut c_void { ptr::null_mut() }
...
define_prim!(hlp_get_screen_width_of_window, "PXsdl_window__i", sdl_get_screen_width_of_window);
define_prim!(hlp_get_screen_height_of_window, "PXsdl_window__i", sdl_get_screen_height_of_window);
...
define_prim!(hlp_set_relative_mouse_mode, "Pb_i", sdl_set_relative_mouse_mode);
define_prim!(hlp_get_relative_mouse_mode, "P_b", sdl_get_relative_mouse_mode);
define_prim!(hlp_warp_mouse_global, "Pii_i", sdl_warp_mouse_global);
define_prim!(hlp_warp_mouse_in_window, "PXsdl_window_ii_v", sdl_warp_mouse_in_window);
...
define_prim!(hlp_get_global_mouse_state, "PRiRi_i", sdl_get_global_mouse_state);
define_prim!(hlp_detect_keyboard_layout, "P_B", sdl_detect_keyboard_layout);
...
define_prim!(hlp_win_get_glcontext, "PXsdl_window__Xsdl_gl_", sdl_win_get_glcontext);
define_prim!(hlp_win_set_fullscreen, "PXsdl_window_i_b", sdl_win_set_fullscreen);
define_prim!(hlp_win_set_display_mode, "PXsdl_window_iii_b", sdl_win_set_display_mode);
define_prim!(hlp_win_display_handle, "PXsdl_window__i", sdl_win_display_handle);
...
define_prim!(hlp_win_set_position, "PXsdl_window_ii_v", sdl_win_set_position);
define_prim!(hlp_win_get_position, "PXsdl_window_RiRi_v", sdl_win_get_position);
define_prim!(hlp_win_set_size, "PXsdl_window_ii_v", sdl_win_set_size);
define_prim!(hlp_win_set_min_size, "PXsdl_window_ii_v", sdl_win_set_min_size);
define_prim!(hlp_win_set_max_size, "PXsdl_window_ii_v", sdl_win_set_max_size);
define_prim!(hlp_win_get_size, "PXsdl_window_RiRi_v", sdl_win_get_size);
define_prim!(hlp_win_get_min_size, "PXsdl_window_RiRi_v", sdl_win_get_min_size);
define_prim!(hlp_win_get_max_size, "PXsdl_window_RiRi_v", sdl_win_get_max_size);
...
define_prim!(hlp_win_render_to, "PXsdl_window_Xsdl_gl__v", sdl_win_render_to);
...
define_prim!(hlp_cursor_create, "PXsdl_surface_ii_Xsdl_cursor_", sdl_cursor_create);
define_prim!(hlp_cursor_create_system, "Pi_Xsdl_cursor_", sdl_cursor_create_system);
...
define_prim!(hlp_set_drag_and_drop_enabled, "Pb_v", sdl_set_drag_and_drop_enabled);
define_prim!(hlp_get_drag_and_drop_enabled, "P_b", sdl_get_drag_and_drop_enabled);
define_prim!(hlp_get_displays, "P_A", sdl_get_displays);
define_prim!(hlp_get_display_modes, "Pi_A", sdl_get_display_modes);
define_prim!(hlp_get_current_display_mode, "Pib_D", sdl_get_current_display_mode);
...
define_prim!(hlp_gctrl_get_button, "PXsdl_gctrl_i_b", sdl_gctrl_get_button);
...
define_prim!(hlp_surface_from, "PBiiiiiii_Xsdl_surface_", sdl_surface_from);
