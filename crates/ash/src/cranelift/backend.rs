...
pub struct CraneliftTierContext {
    ...
    /// Per native index: whether the host takes the arguments as a record
    /// (`native_lib::HostNative::record`).
    native_records: Vec<bool>,
...
impl CraneliftTierContext {
    ...
    pub unsafe fn new(
        ...
        let mut native_records = Vec::with_capacity(bytecode.natives.len());
        for (i, n) in bytecode.natives.iter().enumerate() {
            ...
            native_records.push(ash_core_native_record(&n.lib, &n.name));
        }
    ...
    /// Whether the native takes its arguments as a record of words.
    pub fn native_record(&self, native_idx: usize) -> bool {
        self.native_records
            .get(native_idx)
            .copied()
            .unwrap_or(false)
