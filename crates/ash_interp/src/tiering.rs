...
pub(crate) fn compile_with_cranelift(
    ...
    match result {
        Ok(Ok((addr, mut meta))) => {
            ...
            if ctx.tier_log {
                eprintln!(
                    "[tier] install findex={findex} name={} tier=cranelift addr={addr:#x} ops={} in {:.2}ms on {} t={:.1}ms",
                    ash_core::profile::static_name(findex as u32).unwrap_or("?"),
                    meta.num_ops,
                    t0.elapsed().as_secs_f64() * 1e3,
                    std::thread::current().name().unwrap_or("main"),
                    ash_core::profile::run_elapsed_ms(),
                );
            }
...
pub(crate) fn compile_with_llvm(
    ...
    match result {
        Ok(meta) => {
            ...
            if ctx.tier_log {
                eprintln!(
                    "[tier] install findex={findex} name={} tier=llvm addr={:#x} in {:.2}ms t={:.1}ms",
                    ash_core::profile::static_name(findex as u32).unwrap_or("?"),
                    meta.fn_addr,
                    t0.elapsed().as_secs_f64() * 1e3,
                    ash_core::profile::run_elapsed_ms(),
