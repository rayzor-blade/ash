...
fn main() -> anyhow::Result<()> {
    ...
    for arg in std::env::args().skip(1) {
        ...
        let a = match BytecodeDecoder::decode(p) {
            Ok(a) => a,
            Err(e) => {
                println!("SKIP  {arg}: decode failed: {e}");
                continue;
            }
        };
        ...
        if a.ints != b.ints {
            diffs.push("ints CONTENT".into());
        }
        if a.floats != b.floats {
            diffs.push("floats CONTENT".into());
        }
        if a.strings != b.strings {
            diffs.push("strings CONTENT".into());
        }
        let mut opdiff = 0;
        ...
        if opdiff > 0 {
            diffs.push(format!("{opdiff} functions differ in ops/regs"));
        }
        let mut tydiff = 0;
        for (x, y) in a.types.iter().zip(b.types.iter()) {
            if format!("{:?}", x.kind) != format!("{:?}", y.kind) {
                tydiff += 1;
            }
        }
        if tydiff > 0 {
            diffs.push(format!("{tydiff} type kinds differ"));
        }
        if diffs.is_empty() {
            println!(
                "OK    {arg}  ({} fns, {} types, {} bytes)",
                a.functions.len(),
                a.types.len(),
                bytes.len()
            );
