...
pub fn is_terminator(op: &Opcode) -> bool {
    matches!(
        op,
        Opcode::Ret { .. } | Opcode::Throw { .. } | Opcode::Rethrow { .. } | Opcode::JAlways { .. }
    )
}
...
pub fn successors(op: &Opcode, i: usize, num_ops: usize) -> Vec<usize> {
    match op {
        ...
        Opcode::Switch { offsets, end, .. } => {
            let mut targets: Vec<usize> = offsets
                .iter()
...
mod tests {
    ...
    fn test_is_pure() {
        ...
        assert!(is_pure(&Opcode::Mov {
            dst: Reg(0),
            src: Reg(1)
        }));
        assert!(!is_pure(&Opcode::Call0 {
            dst: Reg(0),
            fun: RefFun(0)
        }));
        assert!(!is_pure(&Opcode::NullCheck { reg: Reg(0) }));
        assert!(!is_pure(&Opcode::SetField {
            obj: Reg(0),
            field: RefField(0),
            src: Reg(1)
        }));
