...
pub unsafe fn hl_aptr<T>(a: *mut varray) -> *mut T {
    unsafe { (a as *mut u8).add(mem::size_of::<varray>()) as *mut T }
}

#[inline]
pub unsafe fn hl_is_ptr(t: *mut hl_type) -> bool {
    unsafe { (*t).kind >= hl_type_kind_HBYTES }
}
...
pub unsafe extern "C" fn hlp_type_size(t: *mut hl_type) -> isize {
    unsafe { T_SIZES[(*t).kind as usize] }
}
            ...
            hl::hl_type_kind_HVIRTUAL => {
                ...
                if to_virt.nfields <= t_virt.nfields {
                    for i in 0..to_virt.nfields as usize {
                        ...
                        if (*f1).hashed_name != (*f2).hashed_name
                            || !hlp_same_type((*f1).t, (*f2).t)
                        {
                            return false;
                        }
                    }
            ...
            hl::hl_type_kind_HOBJ | hl::hl_type_kind_HSTRUCT => {
                ...
                loop {
                    ...
                    if (*sup).kind != hl::hl_type_kind_HOBJ
                        && (*sup).kind != hl::hl_type_kind_HSTRUCT
                    {
                        return false;
                    }
...
pub unsafe extern "C" fn hlp_type_args_count(t: *mut hl::hl_type) -> i32 {
    unsafe {
        ...
        if (*t).kind == hl::hl_type_kind_HFUN
            && let Some(fun) = (*t).__bindgen_anon_1.fun.as_ref()
        {
            return fun.nargs;
        }
        0
...
pub unsafe extern "C" fn hlp_enum_parameters(e: *mut venum) -> *mut varray {
    unsafe {
        ...
        for i in 0..(*c).nparams as usize {
            ...
            *hl_aptr::<*mut vdynamic>(a).add(i) =
                crate::cast::hlp_make_dyn(field, *(*c).params.add(i));
        }
        a
    }
}
    ...
    unsafe {
        ...
        for i in 0..(*tenum).nconstructs as usize {
            ...
            *(hl_aptr::<*mut vbyte>(array).add(i)) =
                (*(*tenum).constructs.add(i)).name as *mut vbyte;
        }
...
#[cfg(test)]
mod guid_name_tests {
    use super::{GUID_NAMES, hlp_register_guid_name};
