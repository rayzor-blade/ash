...
///
/// Borrowed: the interpreter interns its frames and an AOT binary resolves
/// them from a static table, so neither side has any reason to hand over
/// ownership.
#[derive(Debug, Clone, Copy)]
pub struct Frame<'a> {
    pub symbol: &'a str,
    /// The debug path as the compiler recorded it, which may be relative.
...
/// trace instead.
pub fn render(message: &str, frames: &[Frame<'_>]) -> bool {
    render_inner(message, frames).unwrap_or(false)
}

fn render_inner(message: &str, frames: &[Frame<'_>]) -> Option<bool> {
    let colour = std::io::stderr().is_terminal();
    let roots = source_roots();
    let locate = |frame: &Frame<'_>| -> Option<(PathBuf, String)> {
        let path = resolve(frame.file?, &roots)?;
    ...
    let mut located: Vec<(&Frame<'_>, PathBuf, String)> =
