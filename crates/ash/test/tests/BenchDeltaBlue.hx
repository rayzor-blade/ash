...
//
// DeltaBlue constraint solver -- the polymorphic-dispatch benchmark of the
// heavy set. Taken VERBATIM (workload included: 40 x chainTest(100) +
// projectionTest(100)) from rayzor's suite at
// rayzor/compiler/benchmarks/src/deltablue.hx, so this row is directly
// comparable against rayzor's published numbers -- the same reason fib is
// fib(40).
//
// What it adds that the set lacked: method_call measures ONE monomorphic
// call in a flat loop; DeltaBlue is a real class hierarchy (Constraint ->
// Unary/Binary -> Stay/Edit/Equality/Scale) where nearly every hot call is
// a virtual dispatch whose receiver class the compiler cannot prove, plus
// constraint-graph pointer chasing the optimizer cannot scalarize away.
// Guard prints inside the workload turn a wrong propagation into a wrong
// checksum rather than a plausible number.
//
// Adaptation from the rayzor copy is mechanical only: no package, rayzor's
// @:safety metas dropped, trace() -> Sys.println (trace prefixes file:line,
// which the checksum judge must not depend on). The algorithm is untouched.
...
class Constraint {
    ...
    public function addConstraint():Void {
        addToGraph();
        BenchDeltaBlue.planner.incrementalAdd(this);
    }
    ...
    public function satisfy(mark:Int):Constraint {
        ...
        if (!BenchDeltaBlue.planner.addPropagate(this, mark)) trace("Cycle encountered");
        out.mark = mark;
        return overridden;
    }

    public function destroyConstraint():Void {
        if (isSatisfied()) BenchDeltaBlue.planner.incrementalRemove(this);
        removeFromGraph();
    }
}
...
class BenchDeltaBlue {
    ...
    public static function projectionTest(n:Int):Void {
        ...
        if (dst.value != 1170) Sys.println("Projection 1 failed");
        change(dst, 1050);
        total = total + src.value;
        if (src.value != 5) Sys.println("Projection 2 failed");
        change(scale, 5);
        for (i in 0...n - 1) {
            total = total + dests[i].value;
            if (dests[i].value != i * 5 + 1000) Sys.println("Projection 3 failed");
        }
        change(offset, 2000);
        for (i in 0...n - 1) {
            total = total + dests[i].value;
            if (dests[i].value != i * 5 + 2000) Sys.println("Projection 4 failed");
        }
    }
