...
fn generate_hl_bindings(out_dir: &Path) {
    ...
    // This bindgen predates the 2024 edition, whose extern blocks are
    // `unsafe extern`.
    let text = bindings
        .to_string()
    ...
    fs::write(out_dir.join("hl_bindings.rs"), text).expect("Couldn't write HL bindings");
}
...
fn generate_std_symbol_table(out_dir: &Path) {
    ...
    for f in &files {
        ...
        for (i, line) in lines.iter().enumerate() {
            if !matches!(line.trim(), "#[no_mangle]" | "#[unsafe(no_mangle)]") {
                continue;
            }
